...
if (nargin == 2)
   ...
   window = fspecial('gaussian', 11, 1.5);        % 参数�?��标准偏差1.5�?1*11的高斯低通滤波�?
...
window = window/sum(sum(window)); %滤波器归�?��操作�?
 
 
mu1   = filter2(window, img1, 'valid');  % 对图像进行滤波因子加�? valid改成same结果会低�?���?
mu2   = filter2(window, img2, 'valid');  % 对图像进行滤波因子加�?
